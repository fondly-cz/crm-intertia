                ...
                <div class="mt-16 lg:mt-0 space-y-8 transition-all duration-300" :class="isCatalogOpen ? 'lg:col-span-5' : 'lg:col-span-12'">
                    ...
                    <div class="bg-white rounded-[2.5rem] shadow-brand p-10 border border-gray-50 relative overflow-hidden">
                        ...
                        <div class="space-y-6 relative z-10">
                            ...
                            <div v-if="form.errors.customer_email" class="mt-2 text-xs text-red-500 font-bold ml-1">{{ form.errors.customer_email }}</div>
                            ...
                            >
                                <span v-if="form.processing">Ukládám...</span>
...
const companySearchQuery = ref(props.calculation.customer_company || '')
const companySearchResults = ref([])
const isSearching = ref(false)
const showCompanyResults = ref(false)
const selectedCompanyId = ref(props.calculation.company_id || null)
...
onMounted(() => {
    ...
    // Attempt to load employee details if we have an ID
    if (props.calculation.company_employee_id && props.calculation.company_id) {
       // We can search for the specific employee or just use customer_name as proxy
       // but for UI consistency let's mock it for now since we have the data in form
       selectedEmployee.value = {
           id: props.calculation.company_employee_id,
           name: props.calculation.customer_name,
           email: props.calculation.customer_email,
           phone: props.calculation.customer_phone
       }
    }
})
...
const form = useForm({
    ...
    customer_phone: props.calculation.customer_phone,
...
const submit = () => {
    form.patch(`/calculations/${props.calculation.id}`)
}
